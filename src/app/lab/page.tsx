...
export default function LabPage() {
  ...
  useEffect(() => {
    generateSessionId().then((id) => { setSessionId(id); setSessionReady(true); }).catch(() => { setSessionReady(true); });
  }, []);
  ...
  useEffect(() => {
    ...
    const disableDebugger = () => {
      ...
      if (performance.now() - start > 100) {
        document.body.innerHTML = "<h1 style='text-align:center;margin-top:40vh;color:red;'>Access Denied</h1>";
      }
    };
  ...
  const handleQuickTest = async () => {
    if (!uid || !region || !sessionId) return;
    ...
    try {
      const res = await fetch(LOCAL_API_URL, {
        ...
        body: JSON.stringify({ uid, region, fingerprint: sessionId }),
      });
      ...
      setTime(Date.now() - start); setStatus(0);
  ...
  const canTest = mode === "quick" ? !loading && !!uid && !!region && sessionReady : !loading && !!uid && !!region && !!apiKey;
  ...
  const freeCurl = `curl -X POST https://your-domain.com/api/player \\\n  -H "Content-Type: application/json" \\\n  -d '{"uid": "${uid}", "region": "${region}"}'`;
  const freeJs = `const response = await fetch("/api/player", {\n  method: "POST",\n  headers: { "Content-Type": "application/json" },\n  body: JSON.stringify({ uid: "${uid}", region: "${region}" })\n});\nconst data = await response.json();`;
  const apiKeyCurl = `curl -X POST ${API_BASE_URL} \\\n  -H "x-api-key: ${apiKey}" \\\n  -H "Content-Type: application/json" \\\n  -d '{"uid": "${uid}", "region": "${region}"}'`;
  const apiKeyJs = `const response = await fetch("${API_BASE_URL}", {\n  method: "POST",\n  headers: { "Content-Type": "application/json", "x-api-key": "${apiKey}" },\n  body: JSON.stringify({ uid: "${uid}", region: "${region}" })\n});\nconst data = await response.json();`;

  const currentCurl = mode === "quick" ? freeCurl : apiKeyCurl;
  const currentJs = mode === "quick" ? freeJs : apiKeyJs;
